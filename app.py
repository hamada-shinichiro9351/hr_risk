# app.py — 人事リスク・勤怠モニター（フル統合版）

# ===== 起動前提の安全化：UIに原因を出す / Secrets 吸い上げ / パス安定化 =====
...
# Secrets/ENV（未設定でも落とさない）
OPENAI_API_KEY = st.secrets.get("OPENAI_API_KEY", os.getenv("OPENAI_API_KEY"))
if not OPENAI_API_KEY:
    st.info("⚠️ OPENAI_API_KEY が未設定です。LLM所見はルールベースにフォールバックします。")

# 相対パス崩れ対策
BASE = Path(__file__).parent

# ===== 依存の import（UIにスタックトレース表示して停止） =====
try:
    try:
        import plotly.io as pio  # Optional
    except ModuleNotFoundError:
        pio = None  # type: ignore

    import pandas as pd
    from dotenv import load_dotenv
    load_dotenv()  # ローカル実行時のみ有効（Cloudでは Secrets 優先）

    # modules.* の読み込み（ここで失敗したらUIに例外表示）
    ...
    st.error("💥 初期化エラー（modules の import / 初期化時）")
...
- attrition ラベル（任意：1=離職, 0=在籍）
    ...
    if hr_up or use_sample_hr:
        if use_sample_hr:
            hr_df = read_table_auto(str(BASE / "data" / "hr_sample.csv"))
        ...
        try:
            ...
            st.download_button(
                ...
                file_name="attrition_results.xlsx",
...
with tab2:
    ...
    if not _attendance_ok:
        st.error("勤怠モジュールの初期化に失敗しました。以下の例外を確認してください。")
        st.code(f"{type(_attendance_err).__name__}: {_attendance_err}")
        st.stop()
    ...
    if att_up or use_sample_att:
        if use_sample_att:
            att_df = read_table_auto(str(BASE / "data" / "attendance_sample.csv"), parse_dates=["日付"])
        ...
        try:
            ...
            st.download_button(
                "Excelをダウンロード",
                data=att_xls_buf.getvalue(),
                file_name="attendance_anomalies.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )
        except Exception as ex:
            st.warning("Excel出力には openpyxl が必要です。'pip install openpyxl' で追加してください。")
...
4. OpenAIキー（任意）を `.env` または Secrets に設定すると、所見がLLM生成に切り替わります（未設定時はルールベース）。
    """)
